...
export default function List() {
  ...
  const [avisoAEliminar, setAvisoAEliminar] = useState<{ id: string; [key: string]: any } | null>(
    null
  );
  ...
  async function deleteOldAvisos() {
    try {
      ...
      const collectedData: Array<{ id: string; [key: string]: any }> = docRef.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      collectedData.forEach(async (aviso) => {
        if (now - aviso.fechaCreacion >= 365 * 24 * 60 * 60 * 1000) {
          // Si el aviso tiene más de 1 año
      ...
      setAvisos(
        collectedData.filter((aviso) => now - aviso.fechaCreacion < 365 * 24 * 60 * 60 * 1000)
      ); // Actualiza el estado para mostrar solo los avisos recientes
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2">
        {currentAvisos && currentAvisos.length > 0 ? (
          currentAvisos.map((aviso) => (
            <div className="card card-border bg-primary/7 w-100 my-2" key={aviso.id}>
              <div className="card-body">
                ...
                <p>
                  Fecha de creación: {new Date(aviso.fechaCreacion).getDate()}/
                  {new Date(aviso.fechaCreacion).getMonth() + 1}/
                  {new Date(aviso.fechaCreacion).getFullYear()} —
                  {new Date(aviso.fechaCreacion).getHours() >= 10
                    ? new Date(aviso.fechaCreacion).getHours()
                    : '0' + new Date(aviso.fechaCreacion).getHours()}
                  :{new Date(aviso.fechaCreacion).getMinutes().toString().padStart(2, '0')}
                </p>
                <div className="my-1">{renderAdjunto(aviso)}</div>
                  ...
                  <div className="flex gap-1">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        (
                          document.getElementById('modal_confirmar_eliminar') as HTMLDialogElement
                        )?.showModal();
                      }}
                    >
      ...
      <dialog id="modal_confirmar_eliminar" className="modal">
        <div className="modal-box">
          ...
          <div className="modal-action">
            <button className="btn btn-error" onClick={handleDelete} disabled={eliminando}>
              {eliminando ? <span className="loading loading-spinner"></span> : 'Eliminar'}
            </button>
            <form method="dialog">
              <button className="btn btn-ghost" disabled={eliminando}>
                Cancelar
              </button>
            </form>
          </div>
